Shader "Unlit/Texture_Colour_Blinking"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
                ...
                if(_Blink == 1)
                    sineWave = (1.5 * sin((3.0f * _Time.y) / 0.5) + 2.5);
                
                // sample the texture
				fixed4 col = tex2D(_MainTex, i.uv) * _Colour;// + emCol;
                half4 dCol = tex2D(_EmissionMap, i.uv) * _EmissionColour * sineWave;
